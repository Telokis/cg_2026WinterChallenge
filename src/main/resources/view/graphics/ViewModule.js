...
import { easeIn, easeOut } from '../core/transitions.js';
import { fitAspectRatio, lerpPosition, unlerp, unlerpUnclamped } from '../core/utils.js';
import { parseData, parseGlobalData } from './Deserializer.js';
import { TooltipManager } from './TooltipManager.js';
import ev from './events.js';
import { angleBetween, subtract } from './trigo.js';
import { choice, fit, last, setAnimationProgress } from './utils.js';
import { AIGUILLE_FRAME, APPLE_FRAME, AVATAR_RECT, BODY_FRAME, DEATH_FRAMES, EAT_FRAMES, FALL_HEAD_FRAME, GAME_ZONE_RECT, HEAD_FRAME, NAME_RECT, SCORE_RECT, SPARK_FRAMES, TILES, TIME_RECT, TRANSFORM_FRAMES } from './assetConstants.js';
...
export class ViewModule {
    ...
    createEffect(type) {
        ...
        if (type.startsWith('head_')) {
            ...
            display.addChild(id);
            const effect = { pIdx, busy: false, display, faceScaler: scaler, face, eating, dying, id, getTooltip: () => '' };
    ...
    updateMarks() {
        ...
        for (let pIdx = 0; pIdx < this.globalData.playerCount; ++pIdx) {
            const playerMarks = marks[pIdx];
            for (const mark of playerMarks) {
                const markEffect = this.getFromPool('mark');
    ...
    updateApples() {
        ...
        for (let i = 0; i < prevApples.length; ++i) {
            ...
            const effect = this.getFromPool('apple');
    ...
    epsEqual(a, b, eps = 0.0001) {
        return Math.abs(a - b) < eps;
    }
    turnHeadSprite(cuicui, body) {
        const { faceScaler, transformingScaler } = cuicui;
        const dx = body[0].x - body[1].x;
        const dy = body[0].y - body[1].y;
        const angle = Math.atan2(dy, dx);
        const scaler = faceScaler !== null && faceScaler !== void 0 ? faceScaler : transformingScaler;
    ...
    updateBirds() {
        ...
        for (const event of moveEvents) {
            ...
            if (event.growth > 0) {
                const newPart = this.getFromPool(`body_${event.playerId}`);
    ...
    placeInGameZone(display, coord, center = true) {
        ...
        for (let y = 0; y < this.globalData.height; y++) {
            for (let x = 0; x < this.globalData.width; x++) {
                ...
                const neighbours = {
                    ...
                    top: ((_a = this.globalData.map[y - 1]) === null || _a === void 0 ? void 0 : _a[x]) || 0,
                    right: this.globalData.map[y][x + 1] || 0,
                    bottom: ((_b = this.globalData.map[y + 1]) === null || _b === void 0 ? void 0 : _b[x]) || 0,
                    bottomLeft: ((_c = this.globalData.map[y + 1]) === null || _c === void 0 ? void 0 : _c[x - 1]) || 0,
                    bottomRight: ((_d = this.globalData.map[y + 1]) === null || _d === void 0 ? void 0 : _d[x + 1]) || 0,
                };
                let texture = null;
    ...
    handleFrameData(frameInfo, raw) {
        ...
        for (const event of dto.events) {
            if (event.birdId == null)
                continue;
            if (!eventsByBird[event.birdId])
                eventsByBird[event.birdId] = [];
        ...
        for (const event of dto.events) {
            if (event.type === ev.MOVE) {
                ...
                bird.bodyAfterMove = bird.body.map(pos => (Object.assign({}, pos)));
            }
            else if (event.type === ev.FALL) {
                const bird = frameData.birds[event.birdId];
                bird.body = bird.body.map(pos => (Object.assign(Object.assign({}, pos), { y: pos.y + event.numberOfCells })));
            }
